...
internal static partial class NativeCodec
{
    ...
    [LibraryImport("silkcodec", StringMarshalling = StringMarshalling.Utf8)]
    internal static partial int silk_decode(byte[] slk, UIntPtr length, ref IntPtr pcm, ref ulong outlen, int Fs_API);

    [LibraryImport("silkcodec", StringMarshalling = StringMarshalling.Utf8)]
    internal static partial int silk_encode(byte[] pcm, UIntPtr length, ref IntPtr slk, ref ulong outlen, int Fs_API,
        int rate, int packetlength, int complecity,
        int intencent, int loss, int dtx, int inbandfec, int Fs_maxInternal);
}
